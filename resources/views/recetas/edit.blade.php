...
@section('content')
    ...
    <div class="row justify-content-center mt-5">
        <div class="col-md-8">
            <!-- Sent 'el nom del web' => al que equival -->
            <form method="POST" action="{{route('recetas.update', ['receta' => $receta->id])}}" enctype="multipart/form-data" novalidate>
                @csrf

                @method('put') <!-- Indiquem a laravel que serà put -->
                    ...
                    <div class="form-group">
                        ...
                        <select name="categoria" 
                            ...
                            @foreach($categorias as $categoria)
                                <option value="{{$categoria->id}}" {{$receta->categoria_id == $categoria->id ? 'selected' : ''}}>{{$categoria->nombre}}</option>  <!-- Si la categoria que s'introdueix equival a la seva id, queda seleccionada -->
                            @endforeach           
                        </select>
